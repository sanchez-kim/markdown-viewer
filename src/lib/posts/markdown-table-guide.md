...
마크다운 표는 파이프(`|`)로 열을 구분하고, 두 번째 행에 구분 행(`---`)을 넣는 방식입니다.
...
-   `:---` 또는 `:------`: 왼쪽 정렬 (기본값)
-   `---:`: 오른쪽 정렬 (숫자, 금액에 적합)
-   `:---:`: [가운데 정렬](/blog/markdown-center-align) (헤더, 상태 값에 적합)
...
단, [줄바꿈](/blog/markdown-linebreak)은 표 안에서 사용할 수 없습니다. 셀 하나에 여러 줄이 필요하다면 `<br>` HTML 태그를 직접 쓰는 방법이 있지만, 그런 경우엔 표 대신 목록 사용을 고려해보세요.
...
표가 플랫폼마다 다르게 동작하는 이유는 [같은 마크다운인데 플랫폼마다 다르게 보이는 이유](/blog/markdown-flavors-differences)에, 표를 워드·PDF로 낼 때 잘리는 문제는 [내보낼 때 깨지는 것들](/blog/markdown-to-word-pdf)에 정리해두었습니다.
