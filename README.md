...
이 저장소는 **적용 범위가 둘로 나뉩니다.** 자세한 내용은 [LICENSE](LICENSE)를 참조하세요.

| 대상 | 라이선스 |
|---|---|
| 소스 코드·설정 파일 | **MIT** — 자유롭게 사용·수정·배포 가능 |
| `src/lib/posts/` 의 블로그 글 | **저작권 보유(All rights reserved)** — 무단 복제·재발행 금지 |

코드는 누구나 쓰고 검증할 수 있어야 하지만, 글까지 MIT로 열어두면 전문을 그대로
복제해 다른 사이트에 재발행하는 것이 합법이 됩니다. 코드의 개방성은 유지하면서
글의 원본성만 지키기 위한 구분입니다. 인용·전재 문의는 help@easy-md.com 으로 주세요.
